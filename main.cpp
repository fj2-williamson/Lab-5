...
char codeSequence[NUMBER_OF_KEYS]   = { '1', '5', '0', '7' };
...
void alarmActivationUpdate()
{
    ...
    lm35ReadingsAverage = lm35ReadingsSum / NUMBER_OF_AVG_SAMPLES;
       ...
       float tempThreshold = 25 + potentiometer.read() * 12;
       float gasThreshold = potentiometer.read() * 800.0;   
...
void alarmDeactivationUpdate()
{
    ...
    char str[100];
    int stringLength;
    float currentTemp = lm35TempC;
    float currentGas = mq2.read() * 381.4;

float tempThreshold = 25 + potentiometer.read() * 12.0;
                    float gasThreshold  = potentiometer.read() + 600.0;

                    sprintf(str, "Temp threshold: %.2f C\r\n", tempThreshold);
                    uartUsb.write(str, strlen(str));

                    sprintf(str, "Gas threshold: %.0f ppm\r\n", gasThreshold);
                    uartUsb.write(str, strlen(str));
                ...
                for (int i = 0; i < eventsIndex; i++) {
                    ...
                    sprintf (str, "Air quality: %.0f ppm\r\n",currentGas);
                    uartUsb.write( str , strlen(str) );
...
void availableCommands()
{
    ...
    uartUsb.write( "Press '*' to view stored events\r\n\r\n",35);
}
                               ...
                               const char* elementName )
{
    ...
    if ( lastState != currentState ) {
        ...
        arrayOfStoredEvents[eventsIndex].tempLogged = lm35TempC;
        arrayOfStoredEvents[eventsIndex].gasLogged  = mq2.read() * 800.0;
        eventsIndex++;
